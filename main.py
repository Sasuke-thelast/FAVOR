...
administrators = [836931514990198804]
chat_channel = 1048265069999431681
bots_channel = 1048370696306315304
queue = []
...
async def on_ready():
    ...
    while True:
        ...
        activity = discord.Activity(type=discord.ActivityType.watching,
                                    name=f'{members} users | ! 6Followers ')
...
async def on_member_join(member):
    ...
    await channel.send(
        f'Welcome to **6Followers**, {member.mention}.\nType `.help` to get started! Yey!'
    )
    ...
    if ctx.channel.id == bots_channel:
        if isinstance(error, commands.CommandOnCooldown):
            ...
            embed = discord.Embed(
                color=16379747,
                description=
                'You are missing arguments required to run this command!')
            await ctx.send(embed=embed)
...
async def help(ctx):
    ...
    if ctx.channel.type != discord.ChannelType.private:
        ...
        embed.add_field(name='🎉 Twitch Followers',
                        value='`.tfollow (channel)`',
                        inline=True)
        embed.add_field(name='✨ Twitch Spam',
                        value='`.tspam (channel) (message)`',
                        inline=True)
        embed.add_field(name='🔮 Roblox Followers',
...
async def ticket(ctx):
    ...
    if ctx.channel.type != discord.ChannelType.private:
        ...
        if f'ticket-{ctx.author.id}' in str(channels):
            embed = discord.Embed(
                color=16379747, description='You already have a ticket open!')
            await ctx.send(embed=embed)
        else:
            ticket_channel = await ctx.guild.create_text_channel(
                f'ticket-{ctx.author.id}')
            await ticket_channel.set_permissions(ctx.guild.get_role(
                ctx.guild.id),
                                                 send_messages=False,
                                                 read_messages=False)
            ...
            embed = discord.Embed(
                color=16379747,
                description=
                'Please enter the reason for this ticket, type `/close` if you want to close this ticket.'
            )
...
async def tfollow(ctx, channel, amount: int = None):
    ...
    if ctx.channel.type != discord.ChannelType.private:
        if ctx.channel.id == bots_channel or ctx.author.id in administrators:
            if str(channel.lower(
            )) in tfollow_cooldown and ctx.author.id not in administrators:
                try:
                    await ctx.message.delete()
                except:
                    pass
                ...
                try:
                    ...
                    if ctx.author.id in administrators:
                        ...
                        max_amount += 75
                    premium = discord.utils.get(ctx.guild.roles,
                                                name='Premium')
                    if premium in ctx.author.roles:
                        max_amount += 1000
                    diamond = discord.utils.get(ctx.guild.roles,
                                                name='Diamond')
                    ...
                    if amount <= max_amount:
                        ...
                        if premium in ctx.author.roles:
                            ...
                            embed = discord.Embed(
                                color=16777215,
                                description=
                                f'Adding `{amount}` followers to `{channel}`! (`1/{position}`)'
                            )
                            await ctx.send(embed=embed)
                    ...
                    embed = discord.Embed(
                        color=16379747,
                        description=f'Error, try again `{channel}`!')
                    await ctx.send(embed=embed)
                    tfollow.reset_cooldown(ctx)
        else:
            await ctx.message.delete()
                ...
                try:
                    ...
                    if amount <= max_amount:
                        ...
                        embed = discord.Embed(
                            color=16379747,
                            description=
                            f'Added `tspam-{channel}-{msg}` to queue!')
                        await ctx.send(embed=embed)
                        queue.insert(0, f'tspam-{channel}-{msg}')
                except:
                    embed = discord.Embed(
                        color=16379747,
                        description=f'Added `tspam {channel} {msg}` to queue! '
                    )
                    await ctx.send(embed=embed)
                    tspam.reset_cooldown(ctx)
            else:
...
async def rfollow(ctx, user_id, amount: int = None):
    ...
    if ctx.channel.type != discord.ChannelType.private:
        if ctx.channel.id == bots_channel or ctx.author.id in administrators:
            if str(
                    user_id
            ) in rfollow_cooldown and ctx.author.id not in administrators:
                try:
                    await ctx.message.delete()
                except:
                    pass
                ...
                try:
                    ...
                    if amount <= max_amount:
                        ...
                        if premium in ctx.author.roles:
                            ...
                            embed = discord.Embed(
                                color=16777215,
                                description=
                                f'Adding `{amount}` followers to `{user_id}`! (`{position}/{position}`)'
                            )
                            await ctx.send(embed=embed)
                    ...
                    embed = discord.Embed(
                        color=16379747,
                        description=
                        'An error has occured while attempting to run this command!'
                    )
                    await ctx.send(embed=embed)
                    rfollow.reset_cooldown(ctx)
        else:
                ...
                while True:
                    try:
                        ...
                        def check(message: discord.Message):
                            return str(message.content).lower() == str(
                                answer).lower()
                            return SequenceMatcher(
                                None,
                                str(answer).lower(),
                                str(message.content).lower()).ratio() > float(
                                    0.5) and message.channel.id == chat_channel

                        _answer = await bot.wait_for('message',
                                                     check=check,
                                                     timeout=20)
                        try:
                            embed = discord.Embed(
                                color=16379747,
                                description=
                                f'{_answer.author.mention} has answered the question correctly!\n\nAnswer: **{answer}**'
                            )
                            await ctx.send(embed=embed)
                            embed = discord.Embed(
                                color=16379747,
                                description=
                                f'{_answer.author.mention} send your twitch channel to claim the reward!'
                            )
                            await ctx.send(embed=embed)
                            ...
                            _channel = await bot.wait_for('message',
                                                          check=_check,
                                                          timeout=20)
                        ...
                        embed = discord.Embed(
                            color=16379747,
                            description=
                            f'Nobody answered the question correctly.\n\nCorrect Answer: **{answer}**'
                        )
                        await ctx.send(embed=embed)
                    except:
                        pass
